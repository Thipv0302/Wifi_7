...
search over an analytical delay model. We show that the difficulty is not the
cost of that search but its starting point: on the reference five-category
scenario, none of 600 sampled configurations satisfies all five constraints
simultaneously. We therefore replace the search by a pipeline of two graph
neural networks. A proposal network maps a scenario directly to a population of
candidate configurations, including the discrete link assignment, and is trained
...
for the baseline ($p = 0.37$). Evaluation cost was not the binding constraint.

Section~\ref{sec:hard} reports what is. On the five-category scenario
...
crossing the infeasible region, and no acceleration of the fitness function
shortens that crossing. What removes it is a better proposal distribution.
...
\item We show the evolutionary stage is exhausted after one generation, and
report a $10/10$ success rate at $2.6$~s and $142$ analytical-model calls
...
that in this problem it is close to ineffective alone, and that the same network
class applied to the \emph{proposal} is what changes the outcome. The proposal
...
and, even so, spends fewer exact calls than the baseline --- $1{,}070$ and
$474$ against $3{,}362$, as the legend records --- because screening makes each
candidate cheaper. Each curve is the \emph{mean} over ten restarts, shaded by
one standard deviation in (a). The band is itself a result: the pipeline's
deviation never exceeds $0.09$, so its mean is its outcome, while the unseeded
methods scatter by up to $9$ points. A mean is not bounded by the monotonicity
...
$\varepsilon_1$, ten restarts per point. The baseline is run at the search
configuration that reproduces the objective reported for it in~\cite{Yi2025},
about $35$ at $\varepsilon_1 = 10^{-4}$; ours reaches $40.43$ there, the closest
point on our grid and slightly in its favour. Each network is then given the
largest configuration we measured, which still costs it $1{,}070$ and $474$
exact calls against the baseline's $3{,}362$ --- $3.1\times$ and $7.1\times$
...
plotted here. The baseline averages $22.78$ at $\varepsilon_1 = 10^{-8}$ rising
to $40.43$ at $10^{-4}$, the evaluation network $31.73$ to $49.75$ and the
pipeline $47.76 \pm 0.00$ to $50.00 \pm 0.00$: the ordering
$\pi_\psi > g_\phi > \text{baseline}$ holds at all five thresholds while the
call counts run the other way, on five scenarios rather than one, which partly
addresses the limitation in Section~\ref{sec:limits}.
...
uniform rate, is direct evidence that the network has learned the structure of
the constraint set.
...
$58\times$ less time and $206\times$ fewer calls than the baseline.

\subsection{Limitations}
\label{sec:limits}

The $70$-run comparison uses one scenario, which also accounts for half of the
proposal network's training distribution; Fig.~\ref{fig:sweep} adds five
further scenarios but they are drawn from the same family. A sweep over
scenarios absent from the training distribution remains the measurement that
would separate learned structure from memorisation.

Policy training is stochastic in a way that matters. Over five seeded runs at
$1500$ steps all five produce a feasible configuration with $F \in [47.68,
47.76]$, but at $3000$ steps two of five fail and at $6000$ steps the policy
collapses into a degenerate region where $c \to 1$, almost nothing is delivered,
and~\eqref{eq:qos} holds vacuously at $F = 0$. Training length is therefore
bounded above rather than free, and the deployed checkpoint should be selected
across seeds by exact evaluation.

Finally, both networks inherit the analytical model's bias exactly, including
its overestimate of $\Ploss$ relative to slot-level simulation by a factor of
$3$ to $4$. The pipeline accelerates and stabilises the search; it does not
correct the model.

\section{Conclusion}
\label{sec:conclusion}

For multi-link EDCA configuration under delay constraints, the binding cost is
entry into the feasible set rather than the price of a fitness evaluation, which
makes the proposal distribution the place to put model capacity. A pipeline
built on that principle returns the best observed solution in $10/10$ runs in
$2.6$~s using $142$ analytical-model calls, against $152.6$~s, $29{,}309$ calls
and $1/10$ for the genetic baseline. Measured against compute rather than
against search settings, the two networks separate cleanly: the evaluation
...
ceiling, taking the success rate from $8/70$ to $69/70$. Component-wise, an evaluation network that cuts
exact evaluations by $19.8\times$ moves the success rate only from $4/70$ to
$8/70$ ($p = 0.37$); the proposal network moves it to $69/70$. Consistent with
the same reading, the evolutionary stage is exhausted after a single generation.
Extending the evaluation to scenarios outside the training distribution, and to
more than two links, is required before the result can be claimed generally.
